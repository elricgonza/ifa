...
from app import db
from app.models import (Alumno, Asignado, Curso, Grado, Inscrito,
...
def _guard_admin():
    """Guarda de acceso exclusivo para el rol 'administrador' (reportes sensibles)."""
...
def _profesores():
    return (Profesor.query.filter_by(activo=True)
            .order_by(Profesor.paterno, Profesor.nombre).all())
...
def _xhr(ws, s):
    """Como _xh, pero estiliza la última fila añadida en vez de asumir que
    el encabezado está en la fila 1 — útil en hojas con varias secciones."""
...
def r4():
    ...
    # Si llega cur_id usarlo para encontrar la inscripción exacta
    if alu_id and cur_id:
        ins = Inscrito.query.filter_by(alu_id=alu_id, cur_id=cur_id).first()
    elif alu_id:
...
# ═══════════════════════════════════════════════════════════════════════════════
# R9 — Profesores Asignados por Curso  ★ adicional (solo administrador)
# ═══════════════════════════════════════════════════════════════════════════════
...
def r9():
    ...
    orden   = request.args.get('orden',   'curso')
    formato = request.args.get('formato', 'html')
    curso    = Curso.query.get(cur_id) if cur_id else None
    profesor = Profesor.query.get(pro_id) if pro_id else None
    filas = []

    q = (db.session.query(Asignado, Curso, Materia, Profesor)
         .join(Curso,   Asignado.cur_id == Curso.id)
         .join(Materia, Asignado.mat_id == Materia.id)
         .outerjoin(Profesor, Asignado.pro_id == Profesor.id))
    if cur_id: q = q.filter(Asignado.cur_id == cur_id)
    if pro_id: q = q.filter(Asignado.pro_id == pro_id)

    if orden == 'materia':
        q = q.order_by(Materia.materia, Curso.gestion.desc(), Curso.paralelo)
    elif orden == 'profesor':
        q = q.order_by(Profesor.paterno, Profesor.nombre, Curso.gestion.desc())
    else:  # curso
        q = q.order_by(Curso.gestion.desc(), Curso.paralelo, Materia.materia)

    for i, (asig, curso_r, mat, prof) in enumerate(q.all(), 1):
        filas.append({
            'n': i,
            'curso': _nc(curso_r),
            'materia': mat.materia,
            'profesor': prof.nombre_completo if prof else 'Sin asignar',
        })

    if formato == 'excel': return _r9_xl(filas, curso, profesor)
    if formato == 'pdf':   return _r9_pdf(filas, curso, profesor)
    return render_template('reportes/r9.html',
                           filas=filas, cursos=_cursos(), profesores=_profesores(),
                           cur_id=cur_id, pro_id=pro_id,
                           curso=curso, profesor=profesor, orden=orden)


def _r9_nombre(curso, profesor):
    partes = []
    if curso:    partes.append(_nc(curso))
    if profesor: partes.append(profesor.nombre_completo)
    return ('_'.join(partes) if partes else 'todos').replace(' ', '_')


def _r9_xl(filas, curso, profesor):
    from openpyxl import Workbook
    wb = Workbook(); ws = wb.active; ws.title = 'Prof. Asignados'
    s = _xs()
    _xh(ws, ['#', 'Curso', 'Materia', 'Profesor'], s)
    for f in filas:
        ws.append([f['n'], f['curso'], f['materia'], f['profesor']])
        _xrow(ws, s, fill=s['am'] if f['profesor'] == 'Sin asignar' else None)
    _xw(ws)
    return _xsend(wb, f'r9_{_r9_nombre(curso, profesor)}.xlsx')


def _r9_pdf(filas, curso, profesor):
    from reportlab.lib.units import cm
    doc, buf = _psetup(landscape=True)
    partes = []
    if curso:    partes.append(_nc(curso))
    if profesor: partes.append(profesor.nombre_completo)
    subtitulo = ' · '.join(partes) if partes else 'Todos los cursos y profesores'
    story = _ptitle('Profesores Asignados por Curso', subtitulo)
    enc   = [['#', 'Curso', 'Materia', 'Profesor']]
    data  = enc + [[f['n'], f['curso'], f['materia'], f['profesor']] for f in filas]
    story.append(_ptbl(data, [1*cm, 6*cm, 5*cm, 6*cm]))
    return _psend(doc, buf, story, f'r9_{_r9_nombre(curso, profesor)}.pdf')


# ═══════════════════════════════════════════════════════════════════════════════
# R10 — Información Alumno  ★ adicional (solo administrador)
# ═══════════════════════════════════════════════════════════════════════════════
...
def r10():
    ...
    if ins:
        # ── Materias asignadas al curso, con profesor y nota (tablas 'asignado',
        #    'profesor', 'materia' y 'nota') ──
        for asig in Asignado.query.filter_by(cur_id=ins.cur_id).all():
            nota = Nota.query.filter_by(ins_id=ins.id, mat_id=asig.mat_id).first()
            if nota is None:
                estado = 'Sin registrar'
            elif nota.aprobado is None:
                estado = 'Pendiente'
            else:
                estado = 'Aprobado' if nota.aprobado else 'Reprobado'
            materias.append({
                'materia':  asig.materia.materia if asig.materia else f'Materia #{asig.mat_id}',
                'profesor': asig.profesor.nombre_completo if asig.profesor else 'Sin asignar',
                'n1': nota.nota1 if nota else '—',
                'n2': nota.nota2 if nota else '—',
                'n3': nota.nota3 if nota else '—',
                'nf': float(nota.nota_final) if nota else '—',
                'estado': estado,
                'obs': (nota.obs or '') if nota else '',
            })

        # ── Pagos realizados (tabla 'pago') ──
        for p in Pago.query.filter_by(ins_id=ins.id).order_by(Pago.nro_cuota).all():
            pagos.append({
                'nro': p.nro_cuota, 'cuota': p.cuota, 'pagado': p.pagado,
                'metodo': p.metodo_pago or '—',
    ...
    if formato == 'excel': return _r10_xl(alumno, ins, materias, pagos, resumen)
    if formato == 'pdf':   return _r10_pdf(alumno, ins, materias, pagos, resumen)
    return render_template('reportes/r10.html',
                           alumno=alumno, ins=ins, materias=materias,
                           pagos=pagos, resumen=resumen, alu_id=alu_id)


def _r10_xl(alumno, ins, materias, pagos, resumen):
    from openpyxl import Workbook
    wb = Workbook(); ws = wb.active; ws.title = 'Información Alumno'
    s = _xs()
    ws.append(['Alumno:',    alumno.nombre_completo if alumno else ''])
    ws.append(['CI:',        str(alumno.ci) if alumno and alumno.ci else '—'])
    ws.append(['Email:',     (alumno.email or '—') if alumno else '—'])
    ws.append(['Curso:',     _nc(ins.curso) if ins and ins.curso else '—'])
    ws.append(['Descuento:', f'{ins.descuento}%' if ins else '0%'])
    for r in range(1, 6):
        ws[f'A{r}'].font = s['bld']
    ws.append([])

    ws.append(['CALIFICACIONES']); ws[ws.max_row][0].font = s['bld']
    _xh_calif = ['Materia', 'Profesor', 'P1', 'P2', 'P3', 'Promedio', 'Estado', 'Obs.']
    ws.append(_xh_calif); _xhr(ws, s)
    for m in materias:
        ws.append([m['materia'], m['profesor'], m['n1'], m['n2'], m['n3'],
                   m['nf'], m['estado'], m['obs']])
        fill = (s['gf'] if m['estado'] == 'Aprobado' else
                s['rf'] if m['estado'] == 'Reprobado' else
                s['am'] if m['estado'] == 'Pendiente' else None)
        _xrow(ws, s, fill=fill)
    ws.append([])

    ws.append(['PAGOS']); ws[ws.max_row][0].font = s['bld']
    ws.append(['Cuota #', 'Monto Bs.', 'Estado', 'Método',
                'Fecha Pago', 'Referencia', 'Observaciones'])
    _xhr(ws, s)
    for p in pagos:
        ws.append([p['nro'], round(p['cuota'], 2),
                   'Pagado' if p['pagado'] else 'Pendiente',
        ...
        _xrow(ws, s, fill=s['gf'] if p['pagado'] else s['am'])
    ws.append([])
    ws.append(['', 'TOTAL:', round(resumen.get('total', 0), 2), 'PAGADO:',
               round(resumen.get('pagado', 0), 2), 'PENDIENTE:',
               round(resumen.get('pendiente', 0), 2)])
    for c in ws[ws.max_row]: c.font = s['bld']
    _xw(ws)
    fn = alumno.nombre_completo.replace(' ', '_') if alumno else 'alumno'
    return _xsend(wb, f'r10_{fn}.xlsx')


def _r10_pdf(alumno, ins, materias, pagos, resumen):
    from reportlab.lib import colors
    from reportlab.lib.styles import ParagraphStyle
    from reportlab.lib.units import cm
    from reportlab.platypus import Paragraph, Spacer, Table, TableStyle
    doc, buf = _psetup()
    story = _ptitle('Información del Alumno', alumno.nombre_completo if alumno else '')

    info = [['Alumno:',    alumno.nombre_completo if alumno else '—'],
            ['CI:',        str(alumno.ci) if alumno and alumno.ci else '—'],
            ['Email:',     (alumno.email or '—') if alumno else '—'],
            ['Curso:',     _nc(ins.curso) if ins and ins.curso else '—'],
            ['Descuento:', f'{ins.descuento}%' if ins else '0%']]
    it = Table(info, colWidths=[3*cm, 12*cm])
    it.setStyle(TableStyle([('FONTNAME', (0, 0), (0, -1), 'Helvetica-Bold'),
                              ('FONTSIZE', (0, 0), (-1, -1), 9),
                              ('TOPPADDING', (0, 0), (-1, -1), 3),
    ...
    r_, g_, b_ = _PRIM
    pc = colors.Color(r_/255, g_/255, b_/255)
    sub = ParagraphStyle('Sub', fontSize=10, fontName='Helvetica-Bold',
                          textColor=pc, spaceAfter=4)

    story.append(Paragraph('Calificaciones', sub))
    enc_m  = [['Materia', 'Profesor', 'P1', 'P2', 'P3', 'Prom.', 'Estado']]
    data_m = enc_m + [[m['materia'], m['profesor'], m['n1'], m['n2'], m['n3'],
                        m['nf'], m['estado']] for m in materias]
    story.append(_ptbl(data_m, [4*cm, 4.2*cm, 1*cm, 1*cm, 1*cm, 1.8*cm, 2*cm]))
    story.append(Spacer(1, .35*cm))

    story.append(Paragraph('Pagos', sub))
    enc_p  = [['Cuota #', 'Monto Bs.', 'Estado', 'Método', 'Fecha', 'Referencia']]
    data_p = enc_p + [[p['nro'], f"{p['cuota']:.2f}",
                        'Pagado' if p['pagado'] else 'Pendiente',
                        p['metodo'], p['fecha'], p['ref']] for p in pagos]
    story.append(_ptbl(data_p, [1.5*cm, 2.5*cm, 2*cm, 2.5*cm, 2.5*cm, 4*cm]))
    story.append(Spacer(1, .3*cm))
    res = [['Total Bs.', 'Pagado Bs.', 'Pendiente Bs.', 'Cuotas', 'C. Pagadas'],
           [f"{resumen.get('total', 0):.2f}", f"{resumen.get('pagado', 0):.2f}",
            f"{resumen.get('pendiente', 0):.2f}",
    ...
    fn = alumno.nombre_completo.replace(' ', '_') if alumno else 'alumno'
    return _psend(doc, buf, story, f'r10_{fn}.pdf')
...
# ═══════════════════════════════════════════════════════════════════════════════
# AJAX — alumnos filtrados por curso y/o término (para búsqueda en R4)
# ═══════════════════════════════════════════════════════════════════════════════

@reportes_bp.route('/alumnos-search')
@login_required
def alumnos_search():
    _guard()
    cur_id = request.args.get('cur_id', type=int)
    ...
    # Base: alumnos con inscripción
    query = (db.session.query(Alumno, Inscrito, Curso)
             .join(Inscrito, Inscrito.alu_id == Alumno.id)
             .join(Curso,    Curso.id == Inscrito.cur_id)
    ...
    if q:
        ...
        query = query.filter(
            db.or_(
                db.func.lower(Alumno.nombre).like(like),
                db.func.lower(Alumno.paterno).like(like),
                db.func.lower(Alumno.materno).like(like),
                db.cast(Alumno.ci, db.String).like(like),
            )
        )

    query = query.order_by(Alumno.paterno, Alumno.nombre).limit(50)

    result = []
    for alumno, ins, curso in query.all():
        grado = curso.grado.grado if curso and curso.grado else ''
        result.append({
            'id':      alumno.id,
            'nombre':  alumno.nombre_completo,
            'ci':      str(alumno.ci) if alumno.ci else '—',
            'curso':   f"{grado} – {curso.paralelo} ({curso.gestion})" if curso else '—',
            'cur_id':  ins.cur_id,
        })
