...
@nota_bp.route('/inicializar', methods=['GET', 'POST'])
@login_required
def inicializar_notas():
    """Genera automáticamente los registros de la tabla 'nota' a partir de
    'inscrito', 'curso', 'asignado' y 'materia'.

    Para cada alumno inscrito (inscrito.inscrito == True e inscrito.abandono == False)
    en un curso, se obtiene mediante 'asignado' cada materia (y profesor) asignados
    a ese curso, y se crea el registro de nota correspondiente si aún no existe
    (ins_id + mat_id). Solo accesible para el rol 'administrador'."""
    if not current_user.has_role('administrador'):
        abort(403)
    ...
    if request.method == 'POST':
        cur_id = request.form.get('cur_id', type=int) or None

        query = Inscrito.query.filter(Inscrito.inscrito == True, Inscrito.abandono == False)
        if cur_id:
            query = query.filter(Inscrito.cur_id == cur_id)
        inscritos = query.all()
        ...
        sin_asig   = 0
        detalle    = []
        asignaciones_por_curso = {}  # cache: cur_id -> [Asignado, ...]

        for ins in inscritos:
            alumno_nombre = ins.alumno.nombre_completo if ins.alumno else f'Inscrito #{ins.id}'

            if ins.cur_id not in asignaciones_por_curso:
                asignaciones_por_curso[ins.cur_id] = Asignado.query.filter_by(cur_id=ins.cur_id).all()
            asignaciones = asignaciones_por_curso[ins.cur_id]

            if not asignaciones:
                sin_asig += 1
                detalle.append({
                    'alumno': alumno_nombre, 'materia': '—', 'estado': 'sin_asignacion',
                    'msg': 'El curso no tiene materias/profesores asignados.'
                })
                continue

            for asig in asignaciones:
                materia_nombre = asig.materia.materia if asig.materia else f'Materia #{asig.mat_id}'

                ya_existe = Nota.query.filter_by(ins_id=ins.id, mat_id=asig.mat_id).first()
                if ya_existe:
                    omitidas += 1
                    detalle.append({
                        'alumno': alumno_nombre, 'materia': materia_nombre,
                        'estado': 'omitido', 'msg': 'El registro ya existía previamente.'
                    })
                    continue
                ...
                n = Nota(
                    ins_id=ins.id,
                    mat_id=asig.mat_id,
                    nota1=0, nota2=0, nota3=0, nota_final=0,
                    nota_aprob=51,
                    aprobado=False,
                    obs='',
                    creado=datetime.now(), act=datetime.now(),
                ...
                detalle.append({
                    'alumno': alumno_nombre, 'materia': materia_nombre,
                    'estado': 'generado', 'msg': 'Registro creado.'
                })

        db.session.commit()
        log_accion('BULK', 'nota', detalle={
            'accion': 'inicializar_notas', 'cur_id': cur_id,
            'generadas': generadas, 'omitidas': omitidas, 'sin_asig': sin_asig
        })
        flash(
            f'Se generaron {generadas} registro(s) de notas. '
            f'Se omitieron {omitidas} registro(s) que ya existían previamente.',
            'success'
        )

        curso_sel = Curso.query.get(cur_id) if cur_id else None
        return render_template('nota/inicializar_notas.html', ejecutado=True,
                               cursos=cursos, curso_sel=curso_sel,
                               generadas=generadas, omitidas=omitidas, sin_asig=sin_asig,
                               detalle=detalle)

    return render_template('nota/inicializar_notas.html', ejecutado=False, cursos=cursos)
# ── fin inicializar notas ─────────────────────────────────────────────────────
